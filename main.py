...
if __name__ == '__main__':
    ...
    if(args.content_path):
        path = validate_path(args.content_path)
        if (path):
            output = games_loop(path)
    elif(args.game_path):
        path = validate_path(args.game_path)
        if (path):
            output = mods_loop(path)
    elif(args.mod_path):
        path = validate_path(args.mod_path)
        if (path):
            output = mod_lookup(path)
    elif(args.game_id):
        output = game_lookup(args.game_id)
    elif(args.mod_id):
